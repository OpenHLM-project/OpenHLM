#!/usr/bin/env python3
"""
Convert pre-trained PaliGemma model from JAX to PyTorch format.

This script downloads a pre-trained PaliGemma JAX checkpoint and converts it to PyTorch format
using the PI0Pytorch model structure. The converted model can be used for fine-tuning or inference
with PyTorch.

Usage:
    python examples/convert_paligemma_to_pytorch.py --config_name <config_name> --output_path /path/to/output --precision bfloat16

Example:
    # Convert PaliGemma checkpoint to PyTorch
    python examples/convert_paligemma_to_pytorch.py \
        --config_name pi05_aloha \
        --output_path /home/$USER/.cache/openpi/openpi-assets/checkpoints/paligemma_pytorch
"""
...
def slice_paligemma_state_dict(state_dict, config):
    ...
    # Expert-related keys to filter out (these are not present in the base PaliGemma model)
    expert_keys = [
...
def slice_initial_orbax_checkpoint():
    """
    Download and load the pre-trained PaliGemma checkpoint from Google Cloud Storage.
    
    Returns:
        Dictionary containing the PaliGemma parameters from the npz file.
    """
    ...
    paligemma_params = traversals.flatten_mapping(flax.traverse_util.unflatten_dict(flat_params, sep="/")["params"], sep="/")

    return {"paligemma_params": paligemma_params}


def convert_paligemma_to_pytorch(
    precision: str, output_path: str, model_config: openpi.models.pi0_config.Pi0Config
):
    """
    Convert pre-trained PaliGemma JAX checkpoint to PyTorch format.
    ...
    """
    print(f"Converting PaliGemma checkpoint to {output_path}")
    print(f"Model config: {model_config}")

    # Load and slice the pre-trained PaliGemma parameters from npz file
    initial_params = slice_initial_orbax_checkpoint()

    # Create PaliGemma config structure for the pre-trained model
    # This matches the architecture of the base PaliGemma model (paligemma-3b-pt-224)
    ...
    assert len(expert_params) == 0, "Expert parameters should be empty"

    # Instantiate PI0 PyTorch model which wraps PaliGemma
    pi0_model = openpi.models_pytorch.pi0_pytorch.PI0Pytorch(model_config)

    # Use the converted PaliGemma parameters
    all_params = {**paligemma_params}

    # Load state dict into PI0 model (with expert)
    missing_keys, unexpected_keys = pi0_model.load_state_dict(all_params, strict=False)
    if missing_keys:
        print(f"\n=== Keys missing in PI0 model (with expert) - {len(missing_keys)} total ===")
        for key in missing_keys:
            print(f"  Missing: {key}")
    if unexpected_keys:
        print(f"\n=== Unexpected keys from PaliGemma checkpoint - {len(unexpected_keys)} total ===")
    ...
    # Save the converted model using safetensors format
    os.makedirs(output_path, exist_ok=True)

    # Save model weights as SafeTensors (handles tied weights automatically)
...
def main(
    ...
    """Convert pre-trained PaliGemma JAX model to PyTorch format.

    Args:
        config_name: Name of the model configuration to use
        output_path: Path to save converted PyTorch model (required)
        precision: Precision for model conversion (float32, bfloat16, or float16)
    """
    ...
    if not output_path:
        ...
        return
    convert_paligemma_to_pytorch(precision, output_path, model_config)
